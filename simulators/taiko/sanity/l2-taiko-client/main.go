	...
	suite.Add(&hivesim.TestSpec{
		...
		Run: func(t *hivesim.T) {
			...
			if len(c.L2ExecutionClient) != 1 {
				t.Fatal("choose 1 l2_client client type")
			}
			if len(c.L1L2ProtocolDeployerClient) != 1 {
				t.Fatal("choose 1 l1l2_protocol_deployer_client client type")
			}
			if len(c.L2DriverClient) != 1 {
				t.Fatal("choose 1 l2_driver client type")
			}
			if len(c.L2ProposerClient) != 1 {
				t.Fatal("choose 1 l2_proposer client type")
			}
			if len(c.L2ProverClient) != 1 {
				t.Fatal("choose 1 l2_prover client type")
			}
			for _, node := range c.Combinations() {
				...
				config := tn.Config{
					...
					L2ExecutionConsensus: el.ExecutionCliqueConsensus{},
				}
